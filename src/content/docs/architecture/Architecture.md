...
음성 합성은 별도 경로입니다. 사용자가 듣기를 요청하면 Unity가 재생할 캐릭터 발화의 텍스트를 백엔드로 보내고 음성을 받아 재생합니다. 언어 모델 추론이 기기에서 실행된다는 설명이 음성 합성까지 기기 안에서 처리한다는 뜻은 아닙니다.
...
| 입력 | 보관 위치와 사용 방식 |
| --- | --- |
| 최근 대화 | Swift가 관리하고 기기 내 파일에 보존한 최근 대화입니다. 다음 요청의 입력을 구성할 때 사용합니다. |
| 장기 기억 | SQLite에 저장된 발화 중 현재 질문과 관련 있는 기록을 검색해 넣습니다. |
| 현재 발화 | 이번 요청으로 들어온 사용자 입력입니다. |

Swift는 매 요청의 전체 입력을 다시 구성하며, 샘플링 설정과 출력 상한이 호환되면 기존 캐시 세션을 유지합니다. 입력 제출 전에 처리 위치를 되돌리고, LiteRT-LM의 접두부 비교로 같은 토큰 구간을 재사용합니다. 완료된 대화의 상태 저장과 복원은 [KV Cache Reuse and Persistence](?doc=optimization-kv-cache)에서 설명합니다.
...
sequenceDiagram
    ...
    U->>B: 현재 발화와 요청 식별자
    B->>H: 사용자 발화 저장 · 보관 범위 갱신
    Note over B: 도구 분류 결과가 일반 대화인 경우
    B->>M: 최근 문맥의 요청 ID를 제외하고 기억 검색
    M->>E: 검색용 임베딩 요청
    E-->>M: 질문 벡터
    M-->>B: 관련 기억
    B->>E: 장면 분류용 임베딩 요청
    ...
    B->>G: 호환되는 캐시 세션 준비와 시스템 입력 교체
    Note over G: 입력 접두부가 일치하는 KV 재사용
    B->>H: 최근 대화 읽기
    H-->>B: 최근 메시지
...
사용자 발화는 요청을 받아들일 때 최근 대화 파일에 저장합니다. 이 저장에 실패하면 생성을 시작하지 않습니다. 완료·취소·오류 때는 답변과 최종 상태를 확정합니다. 최근 문맥의 수명과 앱 재시작 시 복원 범위는 [Memory](?doc=memory-memory)에서 설명합니다.

답변을 완성하면 `completed` 이벤트를 보내고, 그 뒤 저장 조건을 충족한 사용자 발화를 장기 기억에 기록합니다. 따라서 `completed`는 기억 저장까지 성공했다는 신호는 아닙니다. 장기 기억 쓰기는 트랜잭션으로 묶으며, 실패해도 이미 생성한 답변은 유지됩니다.
...
일반 대화 스트리밍은 현재 생성 작업을 추적해 취소를 요청합니다. 취소가 정해진 시간 안에 끝나지 않으면 런타임을 실패 상태로 바꾸고 오류를 반환합니다. 모델을 해제할 때는 메모리 서비스와 메모리의 세션 상태를 정리합니다. 기기 내 파일에 저장된 최근 대화는 다음 초기화에서 복원합니다.
...
도구 라우팅 후보는 아직 독립적인 평가와 아이폰에서의 전체 실행 검증이 필요합니다. Mac에서 얻은 분류 점수나 생성 시간만으로 아이폰의 지연시간, 메모리 사용량과 연속 사용 시 동작을 판단할 수는 없습니다.
