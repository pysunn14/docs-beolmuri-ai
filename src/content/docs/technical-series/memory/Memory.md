...
| 구분 | 최근 대화 | 장기 기억 |
| --- | --- | --- |
| 보관 위치 | Swift의 메모리에 보관합니다. | SQLite에 기록합니다. |
| 포함 대상 | 최근 사용자 메시지와 표시한 답변을 유지합니다. | 저장 판정을 통과한 사용자 발화를 기록합니다. |
| 선택 기준 | 최근 메시지부터 제한 개수만 유지합니다. | 현재 질문과 벡터 유사도를 비교합니다. |
| 기본 제한 | 최대 20개 메시지입니다. | 검색 결과는 최대 10개이며 입력 길이 제한도 적용합니다. |
| 모델 해제 시 | 최근 대화 기록을 비웁니다. | 서비스 연결을 닫으며 저장 파일과는 수명을 구분합니다. |
...
검색 결과가 10개여도 전부 모델 입력에 들어가지는 않을 수 있습니다. 입력 구성부는 기억 문장들의 UTF-8 바이트 수를 세어 예산 안에 들어오는 항목을 포함합니다. 현재 예산은 10,000입니다. 설정 이름에 토큰이라는 표현이 있지만 실제 계산은 바이트 기준입니다.
