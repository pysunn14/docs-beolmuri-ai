...
이 흐름의 언어 모델 추론은 기기에서 실행됩니다. 듣기 기능은 별도로 캐릭터 발화의 텍스트를 백엔드에 보내 음성을 받습니다. 외부 통신 경계는 [Architecture](?doc=architecture)에서 설명합니다.
...
대화 경로는 전체 입력을 요청마다 구성하면서 호환되는 캐시 세션을 재사용합니다. 앱의 대화 이력은 기기 내 파일에 보존하고, 완료된 대화의 KV 상태도 별도 체크포인트로 저장합니다. 대화 이력은 다음 입력을 결정하며, KV는 계산을 재사용하기 위한 상태입니다. [KV Cache Reuse and Persistence](?doc=optimization-kv-cache)에서 각각의 수명과 복원 절차를 설명합니다.
