...
사용자 발화는 요청을 받아들일 때 최근 대화 파일에 저장하고, 완료·취소·오류 때 답변과 상태를 확정합니다. 장기 기억 저장 여부는 별도 조건으로 결정하므로, 최근 대화에 들어간 모든 메시지가 장기 기억에 기록되는 것은 아닙니다. 저장·복원 범위는 [Memory](?doc=memory-memory)에서 설명합니다.
